...
class MapTest < Minitest::Test
  ...
  def test_lengths
    ...
    lengths = names.map { |name| name.lengths }
  ...
  def test_normalize_zip_codes
    ...
    zip_codes = numbers.map { |zip| zip.rjust(5, '0') }
    assert_equal ["00234", "00010", "09119", "38881"], zip_codes
  end

  def test_backwards
    names = ["alice", "bob", "charlie", "david", "eve"]
    backwards = names.map { |title| title.reverse! }
  ...
  def test_trim_last_letter
    ...
    trimmed = animals.map { |creature| creature.chop }
